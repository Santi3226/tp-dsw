...
const addTipos = async (data) => {
  ...
  try {
    ...
    alert(
      'Tipo de Analisis n°:' + response.data.data.id + ' creado correctamente'
    );
  } catch (error) {
...
const modifyTipos = async (data) => {
  const tiposData = {
    ...
    importe: Number(data.importe) === 0 ? undefined : Number(data.importe),
    plantilaAnalisis:
      Number(data.plantilaAnalisis) === 0
        ? undefined
        : Number(data.plantilaAnalisis),
  };
  try {
    await axiosInstance.put('/tipoAnalisis/' + tiposData.id, tiposData);
