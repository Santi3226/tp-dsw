...
async function findAll(req: Request, res: Response) {
  try {
    const localidades = await em.find(Localidad, {}, { populate: ['centroAtencion'] });
    res.status(200).json({
...
async function findOne(req: Request, res: Response) {
  try {
    ...
    const localidades = await em.findOneOrFail(Localidad, { id },{ populate: ['centroAtencion'] }); //(Localidad, filtros)
    res.status(200).json({
