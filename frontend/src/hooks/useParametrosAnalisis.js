...
try {
    const response = await axiosInstance.post("/parametroAnalisis", parametrosData); 
    alert("Parametro de Analisis n°:" + response.data.data.id +" creado correctamente");
  } catch (error) {
...
const addVinculo = async (data) => {
  const vinculoData = 
    {
    parametroAnalisis: (Number(data.parametroAnalisis)===0 ? undefined : Number(data.parametroAnalisis)),
    tipoAnalisis: (Number(data.tipoAnalisis)===0 ? undefined : Number(data.tipoAnalisis))
    };
try {
    const response = await axiosInstance.post("/tipoAnalisis/vincular/", vinculoData); 
...
const modifyParametros = async (data) => {
  ...
  try {
    ...
    alert("Parametro de Analisis n°:" + parametrosData.id +" modificado correctamente");
  } catch (error) {
...
export {useParametrosAnalisis, deleteParametros, addParametros, modifyParametros, addVinculo};
